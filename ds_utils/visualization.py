...
def sns_clustermap(df):
    return sns.clustermap(df.fillna(df.mean()).select_dtypes(["int", "float"]), z_score=1)

def sns_corrplot(df):
    # add , annot=True for correlation values
    return sns.clustermap(df.select_dtypes(["int", "float"]).corr(), cmap="vlag", vmin=-1, vmax=1)
...
def plot_multiple_histograms(df, lib="seaborn", columns = None):
    ...
    if lib == "ggplot": # not very reliable
        ...
        raise ValueError(f"Parameter 'lib' must libe 'ggplot' or 'seaborn'. Got {lib} instead")
...
def gg_histogram(
    ...
    return (pn.ggplot(df,  pn.aes(var)) +
...
def sns_bivariate_plot(
    ...
    if pd.api.types.is_numeric_dtype(df[x]) and df[x].value_counts().shape[0] > df.shape[0] / 3:
        if qcut:
        ...
        if sort == "yes" or (sort == "categorical" and not pd.api.types.is_numeric_dtype(df[x])):
            sorting = df.groupby(x)[y].agg(agg_fun).sort_values(ascending=ascending).index
            df[x] = pd.Categorical(df[x], categories=sorting)

    if not len(df):
        raise Exception("Empty data frame after filters! Check if min_freq is too high.")

    fig, axes = plt.subplots(2, 1, sharex=True, figsize=(7,5), gridspec_kw={'height_ratios': [1, 4]})
...
def gg_boxplot(
    ...
    df_plot = df.loc[df[x].isin(valid_values), [x, y]]
    if sort:
        sorting = df_plot.groupby(x)[y].agg(agg_fun).sort_values(ascending=ascending).index
        df_plot[x] = pd.Categorical(df_plot[x], categories=sorting)
    if not len(df_plot):
        raise Exception("Empty data frame after filters! Check if min_freq is too high.")
